...
| 7C | Production HSM adapter — Veriscent-hosted Thales payShield 10K via the real 10XPay REST API (mTLS). **Implemented 2026-07-24**: `verify_cvv/4` (`CY`), `verify_arqc/6` + `generate_arpc/6` (`KW`, VISA/MASTERCARD only by default — `VmuCore.FAS.ConfigCatalog`'s `scheme_id_map`), `verify_pin/3` (`BE` — redesigned onto real HSM semantics, see 7E) are all real, working commands built from the real Postman collection + Core Host Commands manual (`pdftotext`-extracted, page references in `ProductionHSM`'s moduledoc), not guessed. **Live connectivity is unverified** — the reference mTLS cert in `slot_1/` has expired (confirmed live: TLS handshake reaches cert exchange, server responds `certificate_unknown`); user will supply a renewed cert to verify. `change_pin/3` and `build_issuer_scripts/2` remain `{:error, :not_implemented}` — `change_pin/3` for a real, now well-understood architectural reason (every real PIN-reference command is PAN-bound by format, but self-service channels never have the real PAN — see the callback's own doc), not a placeholder. Direct-TCP-socket option (`fas/hsm/socket_hsm.ex`) remains an equally real, unbuilt stub. 27/27 new tests (14 `production_hsm_test.exs` — real config resolution incl. a real `ParameterEngine` fix found live (`card_scheme` was never cached, would have silently broken scheme resolution), real Req.Test-faked CY/KW/BE request+response handling; 5 `soft_hsm_pin_test.exs` — real ISO 9564 block round-trip). Full FAS/ASM/CMS/COL/DPS/admin regression before/after: same 10 pre-existing failures, zero regressions. See `docs/compare/Way4_Parity_Implementation_Plan.md` Phase 0 item 7 | `fas/hsm/production_hsm.ex`, `fas/hsm/production_hsm/http_client.ex`, `fas/hsm/socket_hsm.ex`, `fas/config_catalog.ex` | ✅ CY/KW/BE real; change_pin architecturally blocked; live connectivity unverified | 2026-07-24 |
...
| 7E | PIN block verify + try counter — `cms_card_pins` table (pan_token unique, `reference_pin_lmk`, try_counter, pin_locked_at); `CardPin` schema; `maybe_verify_pin/1` in authorization.ex passes DE52 straight to `HSM.verify_pin/3` (never decoded at this layer, unchanged since 7E's original build); increments try_counter on wrong PIN; locks after `max_pin_tries`; RC "55" wrong, "75" locked. **Redesigned 2026-07-24** (Way4 parity plan Phase 0 item 7) — the original `pin_hash`/`pin_salt` design decoded the ISO PIN block to plaintext in application code and compared a PBKDF2 hash, which no real HSM/PCI-compliant flow does. `reference_pin_lmk` (an opaque LMK-encrypted reference, real semantics in `ProductionHSM` via the `BE` command; a dev-only simplification in `SoftHSM`, see its moduledoc) replaces both columns | `fas/hsm/soft_hsm.ex` (verify_pin/change_pin) · `fas/hsm/production_hsm.ex` (verify_pin) · `cms/card_pin.ex` · migration `20260724130001` · `fas/authorization.ex` (unchanged) | ✅ Done, redesigned 2026-07-24 | 2026-07-02 |
...
| 7G | ARQC verification + ARPC generation — `EmvHandler.verify_arqc/1` parses DE55, calls `HSM.verify_arqc/6`; `build_response_de55/3` builds tag 8A (ARC) + tag 91 (ARPC) via `HSM.generate_arpc/6`; `arqc_decline_on_fail` config governs hard-decline vs fail-open. **Arity extended 2026-07-24** — building `ProductionHSM`'s real `KW` command found the real PAN (not just `pan_token`, a one-way hash) is genuinely required for EMV key derivation, and `generate_arpc` also needs the same ATC/UN `verify_arqc` uses (same session key); both were already in scope at the caller, just not forwarded — a real, found-live bug, not a redesign | `fas/emv_handler.ex`, `fas/hsm/hsm.ex` | ✅ Done | 2026-07-02 |
