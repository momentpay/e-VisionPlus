defmodule VmuCore.HCS.SpendingControl do
  ...
  schema "hcs_spending_controls" do
    field :scope,            :string        # COMPANY | EMPLOYEE | FLEET
    field :company_id,       :integer
    field :employee_card_id, :integer
    field :fleet_card_id,    :integer
  ...
  @valid_scopes        ~w(COMPANY EMPLOYEE FLEET)
  ...
  def changeset(control, attrs) do
    control
    |> cast(attrs, [:scope, :company_id, :employee_card_id, :fleet_card_id, :control_type,
                    :mcc_codes, :channels, :daily_cap, :per_txn_cap, :effective_from,
                    :effective_to, :status, :inserted_at])
