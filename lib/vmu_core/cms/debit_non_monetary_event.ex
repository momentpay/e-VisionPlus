defmodule VmuCore.CMS.DebitNonMonetaryEvent do
  @moduledoc """
  Non-monetary Debit account maintenance events — Card Products UX
  Parity Phase 1e (2026-07-28), mirrors `CMS.NonMonetaryEvent` exactly
  (own table, own FK). A smaller event-type set than Credit's: no
  `cycle_change` (no billing cycle), no `card_reissue` (Debit's
  replace/renew is deliberately not built yet — see `DebitComponent`'s
  own moduledoc), no `pin_change` (no PIN system exists for any product
  ...
  schema "cms_debit_non_monetary_events" do
  ...
  @required [:debit_account_id, :event_type, :operator_id, :applied_at]
  ...
  def history_for(debit_account_id) do
    Repo.all(
      from e in __MODULE__,
        where: e.debit_account_id == ^debit_account_id,
