...
| 7C | Production HSM adapter — skeleton with `{:error, :not_implemented}` on all callbacks + Logger.warning. **Vendor/approach decided 2026-07-24** (still stub, not implemented): Veriscent-hosted Thales payShield 10K via the real 10XPay REST API (mTLS, one endpoint per host command — `CY`→`verify_cvv/4` confirmed, ARQC/ARPC endpoints confirmed, PIN/issuer-script command mapping still needs picking from the real Host Commands manual). Reference material + real mTLS credentials: `D:\momentPay\Products\E-VisionPlus\Veriscent-HSM-cloud\` (never copy `slot_1/` into this repo). A parallel direct-TCP-socket option is kept as an equally real, equally unbuilt stub (`fas/hsm/socket_hsm.ex`) rather than committing to REST-only before deployment topology is known — see `docs/compare/Way4_Parity_Implementation_Plan.md` Phase 0 item 7 | `fas/hsm/production_hsm.ex`, `fas/hsm/socket_hsm.ex` | ✅ stub, 🔜 real impl not started | 2026-07-02 |
