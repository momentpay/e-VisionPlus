defmodule VmuCoreWeb.Live.Admin.DebitComponent do
  ...
  def handle_event("card_activate", %{"id" => card_id}, socket) do
    if socket.assigns.can_edit do
      case CardLifecycle.activate(card_id, operator: socket.assigns.current_operator.username) do
        {:ok, _card} ->
  ...
  def handle_event("card_block", %{"id" => card_id}, socket) do
    if socket.assigns.can_edit do
      case CardLifecycle.block(card_id, "ADMIN", operator: socket.assigns.current_operator.username) do
        {:ok, _card} ->
  ...
  def handle_event("card_unblock", %{"id" => card_id}, socket) do
    if socket.assigns.can_edit do
      case CardLifecycle.unblock(card_id, operator: socket.assigns.current_operator.username) do
        {:ok, _card} ->
